...
- Les catégories (y compris `Parent > Enfant`), marques, étiquettes et attributs globaux doivent déjà exister dans la boutique. Les noms de marques et catégories différents du CSV peuvent être associés manuellement aux destinations du site. Les catégories manquantes peuvent être créées explicitement à la racine ou sous le groupe choisi depuis l’écran de correspondances ; les autres référentiels doivent déjà exister.
...
Sélectionnez une ligne. Pour une marque, choisissez sa destination WooCommerce dans la première liste. Pour une catégorie, choisissez d’abord la marque ou catégorie principale, puis sa catégorie de destination dans la seconde liste. La première liste suit la hiérarchie des catégories du site : elle ne confond pas ces groupes avec la taxonomie des marques. « Toutes les catégories » permet de consulter toute la hiérarchie. Les chemins complets distinguent les noms identiques sous différents parents. Vous pouvez corriger n’importe quelle proposition, même une correspondance exacte. Cliquez sur **Mémoriser et valider ces correspondances** pour poursuivre le contrôle des articles. Une suggestion ne devient jamais une association sans cette validation. Une ligne sans destination bloque la validation. Pour une catégorie absente, cliquez sur **Nouvelle catégorie…** puis saisissez son nom. La fenêtre indique le groupe parent : la catégorie est créée sous ce groupe, ou à la racine si « Toutes les catégories » est sélectionné. Une catégorie du même nom sous le même parent est réutilisée. Cette action modifie réellement la boutique, avant la validation des correspondances. La fenêtre dispose d’un défilement pour conserver l’accès aux boutons sur les petits écrans.
...
### Noms, couleurs et catégories par modèle

Après sélection du **document fournisseur original**, ouvrez **Règles produits…** dans l’écran principal. Cet éditeur de catalogues permet de choisir le champ du modèle, le format du nom (`{model} — {color}`), une catégorie proposée par modèle, puis de corriger les groupes et les traductions de couleurs. Les correspondances entre catégories proposées et catégories de la boutique sont ensuite gérées dans l’écran d’import.

Les règles sont enregistrées par fournisseur et par boutique configurée, dans le dossier local `regles-produits/` à côté de `shop.json`. Les règles spécifiques d’un fournisseur peuvent aussi être fournies par son profil privé. Une couleur inconnue bloque la préparation avec un message invitant à compléter sa traduction. Les références, codes couleur et EAN utilisés pour associer les images restent ceux du fournisseur. Les fiches restent des produits simples distincts par couleur : aucune variation WooCommerce n’est créée.

Après enregistrement des règles, relancez la préparation. Pour un parcours en deux étapes déjà en pause, la précédente préparation est sauvegardée dans `sauvegarde_preparation_*` avant son remplacement. Le ZIP d’images pourra ensuite être utilisé pour finaliser le nouveau CSV. Les produits déjà présents sur la boutique restent ignorés par l’import en ajout uniquement.
...
À chaque passage de dev vers main, incrémenter la version applicative dans `core/version.py`, actualiser ces notes et créer le tag correspondant après validation. La prochaine livraison après 0.27 sera 0.28, sauf demande explicite différente.
...
Lorsqu’un pack privé rencontre des fichiers locaux différents, un bouton permet de les remplacer explicitement par le pack téléchargé. Les fichiers concernés sont sauvegardés dans un dossier `private/pack-backup-*` avant toute modification. Une sauvegarde impossible bloque le remplacement. Les autres fichiers locaux restent conservés. Redémarrer l’application après réception du pack.
...
Lorsque le pack privé est déjà à jour et qu’aucun fichier n’est modifié, le bouton Fermer et la croix ferment uniquement la fenêtre de mise à jour. L’application reste utilisable. Un redémarrage reste demandé lorsque des fichiers du pack ont changé.
