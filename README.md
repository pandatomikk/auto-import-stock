...
Les accès peuvent être mémorisés avec **Mémoriser les accès sur ce poste** puis **Enregistrer les accès** (également appliqué au test de connexion). Ils sont rechargés après fermeture ou mise à jour. Le fichier est `~/.config/auto-import-stock/shop.json` (ou `$XDG_CONFIG_HOME/auto-import-stock/shop.json`) sous Linux, et `%LOCALAPPDATA%/auto-import-stock/shop.json` sous Windows, hors du dépôt et de l’installation. Il contient les secrets en clair : ne le partagez pas. Sous Linux, il est créé avec les permissions `0600` ; sous Windows, il hérite des protections du dossier utilisateur. Pour retirer les secrets enregistrés, décochez l’option puis enregistrez. Les accès déjà perdus devront être saisis une dernière fois.
